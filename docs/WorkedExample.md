...
We see that the `Partition` used was `yew-himem`. Referring to the file `cluster_info__demo.yaml` in the `configuration/examples` subdirectory, we see that the type of CPU model used was a Xeon Gold 6142, with TDP (Thermal Design Power) of 9.4 Watts per core. This is *P*<sub>c</sub>.
...
Looking at the file `cluster_info__demo.yaml` in the directory `configuration/examples`, we find a value for *CI* = 231.12 gCO<sub>2</sub>e kWh<sup>−1</sup>. Hence, the carbon footprint of the energy of:
